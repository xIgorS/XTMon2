...
public partial class ReplayFlows : ComponentBase, IAsyncDisposable
{
    ...
    // Normalized DB status values matched in GetStatusKind (separators stripped, compared case-insensitively)
    ...
    private const string NormalizedSubmissionStartedTypo = "submissonstarted"; // DB typo, intentional — compensates for typo in the DB status value
    ...
    [Inject]
    private ReplayFlowRepository Repository { get; set; } = default!;
    ...
    {
        return value is { } v ? DateOnly.FromDateTime(v).ToString(DisplayDateFormat, CultureInfo.InvariantCulture) : "-";
    }

    private static string FormatDateTime(DateTime value)
    {
        return value.ToString(DisplayDateTimeFormat, CultureInfo.InvariantCulture);
    }
    ...
    private async Task LoadStatusAsync()
    {
        try
        {
            ...
            pendingCount = statusRows.Count(r => GetStatusKind(r) == ReplayStatusKind.Pending);
            inProgressCount = statusRows.Count(r => GetStatusKind(r) == ReplayStatusKind.InProgress);
            completedCount = statusRows.Count(r => GetStatusKind(r) == ReplayStatusKind.Completed);
        }
        catch (OperationCanceledException) when (disposeCts.IsCancellationRequested)
        {
    ...
    private static string FormatDuration(ReplayFlowStatusRow row)
    {
        ...
        if (row.DateStarted is null) return "-";

        var end = row.DateCompleted ?? DateTime.UtcNow;
        var duration = end - row.DateStarted.Value;
    ...
    {
        return GetStatusKind(row) == ReplayStatusKind.Completed;
    }

    private enum ReplayStatusKind
    {
        Pending,
        InProgress,
        Completed
