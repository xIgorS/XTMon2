...
public partial class RolloveredPortfolios : ComponentBase, IDisposable
{
    ...
    private const string MonitoringSubmenuKey = "rollovered-portfolios";

    private readonly HashSet<DateOnly> availableDates = [];

    [Inject]
    private IRolloveredPortfoliosRepository Repository { get; set; } = default!;
    ...
    [Inject]
    private ILogger<RolloveredPortfolios> Logger { get; set; } = default!;
    ...
    private async Task RunAsync()
    {
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ...
                ex,
                "Failed to load Rollovered Portfolios for PnlDate {PnlDate}.",
                selectedPnlDate.Value);
    ...
    private async Task RestoreLatestJobAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to restore latest Rollovered Portfolios job for PnlDate {PnlDate}.", selectedPnlDate.Value);
        }
    }
    ...
    private async Task CopySqlToClipboardAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to copy Rollovered Portfolios SQL statement to clipboard.");
