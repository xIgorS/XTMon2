...
public partial class MultipleFeedVersion : ComponentBase, IDisposable
{
    ...
    private const string MonitoringSubmenuKey = "multiple-feed-version";
    ...
    [Inject]
    private IMultipleFeedVersionRepository Repository { get; set; } = default!;
    ...
    [Inject]
    private ILogger<MultipleFeedVersion> Logger { get; set; } = default!;
    ...
    private async Task RunAsync()
    {
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ...
                ex,
                "Failed to load Multiple Feed Version for PnlDate {PnlDate}.",
                selectedPnlDate.Value);
    ...
    private async Task RestoreLatestJobAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to restore latest Multiple Feed Version job for PnlDate {PnlDate}.", selectedPnlDate.Value);
        }
    }
    ...
    private async Task CopySqlToClipboardAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to copy Multiple Feed Version SQL statement to clipboard.");
