...
public partial class PublicationConsistency : ComponentBase, IDisposable
{
    ...
    private const string MonitoringSubmenuKey = "publication-consistency";
    ...
    [Inject]
    private IPublicationConsistencyRepository Repository { get; set; } = default!;
    ...
    [Inject]
    private ILogger<PublicationConsistency> Logger { get; set; } = default!;
    ...
    private async Task RunAsync()
    {
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ...
                ex,
                "Failed to load Publication Consistency for PnlDate {PnlDate}.",
                selectedPnlDate.Value);
    ...
    private async Task RestoreLatestJobAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to restore latest Publication Consistency job for PnlDate {PnlDate}.", selectedPnlDate.Value);
        }
    }
    ...
    private async Task CopySqlToClipboardAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to copy Publication Consistency SQL statement to clipboard.");
