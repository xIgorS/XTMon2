...
public partial class DataValidationRunner : ComponentBase, IDisposable
{
    ...
    private async Task DeterminePermissionsAsync()
    {
        ...
        var authorizationResult = await AuthorizationService.AuthorizeAsync(authState.User, resource: null, policyName: "UamRestricted");
        canRunRestrictedChecks = authorizationResult.Succeeded;
    }
    ...
    private void BuildRows()
    {
        ...
        foreach (var check in DataValidationCheckCatalog.Checks)
        {
            if (check.RequiresRestrictedAccess && !canRunRestrictedChecks)
            {
                hasHiddenRestrictedChecks = true;
                continue;
            }

            rows.Add(new BatchRunRow(check));
        }

        selectableRows.Clear();
        selectableRows.AddRange(rows);
    ...
    private async Task RefreshStatusesCoreAsync()
    {
        ...
        try
        {
            ...
            var jobLookup = jobs.ToDictionary(
                static job => MonitoringJobHelper.BuildDataValidationSubmenuKey(job.SubmenuKey),
                StringComparer.OrdinalIgnoreCase);

            foreach (var row in rows)
            {
                jobLookup.TryGetValue(row.Definition.Route, out var latestJob);
        ...
        {
            Logger.LogWarning(ex, "Unable to refresh Data Validation batch-run statuses for PnlDate {PnlDate}.", selectedPnlDate);
    ...
    private async Task RunSelectedAsync()
    {
        ...
        var failedCount = 0;
        var defaultPrecision = JvBalanceConsistencyOptions.Value.Precision;
        ...
        foreach (var row in selectedRows)
        {
            try
            {
                var enqueueResult = await MonitoringJobRepository.EnqueueMonitoringJobAsync(
                    MonitoringJobHelper.DataValidationCategory,
                    row.Definition.Route,
                    row.Definition.DisplayName,
                    selectedPnlDate.Value,
                    DataValidationBatchRunHelper.BuildDefaultParametersJson(row.Definition.Route, defaultPrecision),
                    DataValidationBatchRunHelper.BuildDefaultParameterSummary(row.Definition.Route, defaultPrecision),
                    disposeCts.Token);
            ...
            {
                Logger.LogError(ex, "Failed to enqueue Data Validation batch item {Route} for PnlDate {PnlDate}.", row.Definition.Route, selectedPnlDate.Value);
    ...
    {
        return DataValidationNavAlertHelper.GetRunState(job) switch
        {
    ...
    private static string GetStatusLabel(MonitoringJobRecord? job)
    {
        ...
        var runState = DataValidationNavAlertHelper.GetRunState(job);
    ...
    {
        public BatchRunRow(DataValidationCheckDefinition definition)
        {
            Definition = definition;
        }

        public DataValidationCheckDefinition Definition { get; }
