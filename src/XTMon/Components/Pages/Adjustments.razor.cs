...
public partial class Adjustments : ComponentBase, IDisposable
{
    ...
    private const string MonitoringSubmenuKey = "adjustments";
    ...
    private async Task RestoreLatestJobAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to restore latest Adjustments job for PnlDate {PnlDate}.", selectedPnlDate.Value);
        }
    }
    ...
    private void ApplyJob(MonitoringJobRecord job)
    {
        ...
        {
            runError = MonitoringDisplayHelper.GetSafeBackgroundJobMessage(job.ErrorMessage, AdjustmentsLoadErrorMessage);
        }
        else
        {
            runError = null;
