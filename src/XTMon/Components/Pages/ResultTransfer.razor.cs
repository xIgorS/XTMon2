...
public partial class ResultTransfer : ComponentBase, IDisposable
{
    ...
    private const string MonitoringSubmenuKey = "result-transfer";

    private readonly HashSet<DateOnly> availableDates = [];

    [Inject]
    private IResultTransferRepository Repository { get; set; } = default!;
    ...
    [Inject]
    private ILogger<ResultTransfer> Logger { get; set; } = default!;
    ...
    private async Task RunAsync()
    {
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ...
                ex,
                "Failed to load Result Transfer for PnlDate {PnlDate}.",
                selectedPnlDate.Value);
    ...
    private async Task RestoreLatestJobAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to restore latest Result Transfer job for PnlDate {PnlDate}.", selectedPnlDate.Value);
        }
    }
    ...
    private async Task CopySqlToClipboardAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to copy Result Transfer SQL statement to clipboard.");
