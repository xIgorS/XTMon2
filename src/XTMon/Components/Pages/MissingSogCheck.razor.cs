...
public partial class MissingSogCheck : ComponentBase, IDisposable
{
    ...
    private const string MonitoringSubmenuKey = "missing-sog-check";

    private readonly HashSet<DateOnly> availableDates = [];

    [Inject]
    private IMissingSogCheckRepository Repository { get; set; } = default!;
    ...
    [Inject]
    private ILogger<MissingSogCheck> Logger { get; set; } = default!;
    ...
    private async Task RunAsync()
    {
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ...
                ex,
                "Failed to load Missing SOG Check for PnlDate {PnlDate}.",
                selectedPnlDate.Value);
    ...
    private async Task RestoreLatestJobAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to restore latest Missing SOG Check job for PnlDate {PnlDate}.", selectedPnlDate.Value);
        }
    }
    ...
    private async Task CopySqlToClipboardAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to copy Missing SOG Check SQL statement to clipboard.");
