...
public partial class FutureCash : ComponentBase, IDisposable
{
    ...
    private const string LoadErrorMessage = "Unable to load Future Cash right now. Please try again.";
    private const string MonitoringSubmenuKey = "future-cash";
    ...
    [Inject]
    private IFutureCashRepository Repository { get; set; } = default!;
    ...
    [Inject]
    private IOptions<FutureCashOptions> FutureCashOptions { get; set; } = default!;

    [Inject]
    private IOptions<MonitoringJobsOptions> MonitoringJobsOptions { get; set; } = default!;

    [Inject]
    private ILogger<FutureCash> Logger { get; set; } = default!;
    ...
    private string ProcedureName => FutureCashOptions.Value.FutureCashStoredProcedure;
    private string FullyQualifiedProcedureName => JvCalculationHelper.BuildFullyQualifiedProcedureName(FutureCashOptions.Value.ConnectionStringName, ProcedureName);
    ...
    private async Task RunAsync()
    {
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ...
                ex,
                "Failed to load Future Cash for PnlDate {PnlDate}.",
                selectedPnlDate.Value);
    ...
    private async Task RestoreLatestJobAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to restore latest Future Cash job for PnlDate {PnlDate}.", selectedPnlDate.Value);
        }
    }
    ...
    private async Task CopySqlToClipboardAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to copy Future Cash SQL statement to clipboard.");
