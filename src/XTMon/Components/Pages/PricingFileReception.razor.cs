...
public partial class PricingFileReception : ComponentBase, IDisposable
{
    ...
    private async Task RunAsync()
    {
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ...
                ex,
                "Failed to load Pricing File Reception for PnlDate {PnlDate}.",
                selectedPnlDate.Value);
            runError = PricingFileReceptionLoadErrorMessage;
    ...
    private void ApplyJob(MonitoringJobRecord job)
    {
        ...
        {
            runError = MonitoringDisplayHelper.GetSafeBackgroundJobMessage(job.ErrorMessage, PricingFileReceptionLoadErrorMessage);
        }
        else
        {
            runError = null;
    ...
    private async Task CopySqlToClipboardAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to copy Pricing File Reception SQL statement to clipboard.");
