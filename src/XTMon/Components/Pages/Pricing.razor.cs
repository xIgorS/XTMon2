...
public partial class Pricing : ComponentBase
{
    ...
    private async Task LoadPnlDatesAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to load default PNL dates for Pricing.");
        }
    }
    ...
    private async Task RunAsync()
    {
        ...
        try
        {
            var response = await Repository.GetPricingAsync(selectedPnlDate.Value, GetSelectedSourceSystemCodes(), CancellationToken.None);
    ...
    private static string FormatCellValue(string columnName, string? value)
    {
        ...
        if (string.Equals(MonitoringDisplayHelper.NormalizeColumnName(columnName), "pnldate", StringComparison.Ordinal) &&
