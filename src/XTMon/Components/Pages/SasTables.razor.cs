...
public partial class SasTables : ComponentBase, IDisposable
{
    ...
    private const string LoadErrorMessage = "Unable to load Sas tables right now. Please try again.";
    private const string MonitoringSubmenuKey = "sas-tables";

    private readonly HashSet<DateOnly> availableDates = [];

    [Inject]
    private ISasTablesRepository Repository { get; set; } = default!;
    ...
    [Inject]
    private IOptions<SasTablesOptions> SasTablesOptions { get; set; } = default!;

    [Inject]
    private IOptions<MonitoringJobsOptions> MonitoringJobsOptions { get; set; } = default!;

    [Inject]
    private ILogger<SasTables> Logger { get; set; } = default!;
    ...
    private string ProcedureName => SasTablesOptions.Value.SasTablesStoredProcedure;
    private string FullyQualifiedProcedureName => JvCalculationHelper.BuildFullyQualifiedProcedureName(SasTablesOptions.Value.ConnectionStringName, ProcedureName);
    ...
    private async Task RunAsync()
    {
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ...
                ex,
                "Failed to load Sas tables for PnlDate {PnlDate}.",
                selectedPnlDate.Value);
    ...
    private async Task RestoreLatestJobAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to restore latest Sas tables job for PnlDate {PnlDate}.", selectedPnlDate.Value);
        }
    }
    ...
    private async Task CopySqlToClipboardAsync()
    {
        ...
        {
            Logger.LogWarning(ex, "Unable to copy Sas tables SQL statement to clipboard.");
