...
/****** Object:  StoredProcedure [monitoring].[UspCheckJvCalculation]    Script Date: 2/27/2026 7:10:42 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [monitoring].[UspCheckJvCalculation] 
@PnlDate DATE, @Execute bit = 1, @Query NVARCHAR(MAX) = '' OUTPUT,
@Precision DECIMAL(5, 2) = '0.01'
AS
/*
declare @Query nvarchar(max);
EXEC [monitoring].[UspCheckJvCalculation] @PnlDate = '2026-02-13', @Execute = 1, @Query = @Query OUTPUT
select @Query
*/
BEGIN

--wait for 25 seconds to ensure that the JvCalculation is completed before running the check
 WAITFOR DELAY '00:00:25';

select @Query = 'SELECT @PubliBehavior=[PubliBehavior]  from  [administration].[Publ_Parameters] with (NOLOCK);
...
SELECT [PnlDate]
      ,[JvReferentialSettingsId]
      ,[PortfolioSourceId]
      ,[PortfolioDestinationId]
      ,[DailyAmountHO]
      ,[MtdAmountHO]
      ,[YtdAmountHO]
      ,[DailyAmountParadigm]
