...
public sealed class AdjustmentsRepository : IAdjustmentsRepository
{
    private readonly SqlConnectionFactory _connectionFactory;
    private readonly AdjustmentsOptions _options;
    private readonly ILogger<AdjustmentsRepository> _logger;

    public AdjustmentsRepository(
        SqlConnectionFactory connectionFactory,
        IOptions<AdjustmentsOptions> options,
        ILogger<AdjustmentsRepository> logger)
    {
    ...
    public async Task<IReadOnlyList<AdjustmentsSourceSystem>> GetSourceSystemsAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var sourceSystems = new List<AdjustmentsSourceSystem>();
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                sourceSystems.Add(new AdjustmentsSourceSystem(sourceSystemCode));
            }
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(
                ...
                ex,
                "Adjustments source-system procedure execution failed for {StoredProcedure}.",
                _options.GetAllSourceSystemsStoredProcedure);
            throw;
        }
    }

    public async Task<AdjustmentsResult> GetAdjustmentsAsync(DateOnly pnlDate, string? sourceSystemCodes, CancellationToken cancellationToken)
    {
        try
        {
            using var connection = _connectionFactory.CreateConnection(_options.ConnectionStringName);
            using var command = connection.CreateCommand();
            command.CommandText = _options.AdjustmentsStoredProcedure;
            command.CommandType = CommandType.StoredProcedure;
            command.CommandTimeout = _options.CommandTimeoutSeconds;
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(
                AppLogEvents.RepositoryMonitoringProcedureFailed,
                ex,
                "Adjustments procedure execution failed for {StoredProcedure}, PnlDate {PnlDate}, SourceSystemCodes {SourceSystemCodes}.",
                _options.AdjustmentsStoredProcedure,
                pnlDate,
                sourceSystemCodes);
            throw;
        }
