...
public sealed class ResultTransferRepository : IResultTransferRepository
{
    private readonly SqlConnectionFactory _connectionFactory;
    private readonly ResultTransferOptions _options;
    private readonly ILogger<ResultTransferRepository> _logger;

    public ResultTransferRepository(
        SqlConnectionFactory connectionFactory,
        IOptions<ResultTransferOptions> options,
        ILogger<ResultTransferRepository> logger)
    {
    ...
    public async Task<ResultTransferResult> GetResultTransferAsync(DateOnly pnlDate, CancellationToken cancellationToken)
    {
        try
        {
            using var connection = _connectionFactory.CreateConnection(_options.ConnectionStringName);
            using var command = connection.CreateCommand();
            command.CommandText = _options.ResultTransferStoredProcedure;
            ...
            return new ResultTransferResult(
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(
                ...
                ex,
                "Result Transfer procedure execution failed for {StoredProcedure}, PnlDate {PnlDate}.",
                _options.ResultTransferStoredProcedure,
                pnlDate);
            throw;
        }
    }
