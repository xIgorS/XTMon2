...
public sealed class VrdbStatusRepository : IVrdbStatusRepository
{
    private readonly SqlConnectionFactory _connectionFactory;
    private readonly VrdbStatusOptions _options;
    private readonly ILogger<VrdbStatusRepository> _logger;

    public VrdbStatusRepository(
        SqlConnectionFactory connectionFactory,
        IOptions<VrdbStatusOptions> options,
        ILogger<VrdbStatusRepository> logger)
    {
    ...
    public async Task<VrdbStatusResult> GetVrdbStatusAsync(DateOnly pnlDate, CancellationToken cancellationToken)
    {
        try
        {
            using var connection = _connectionFactory.CreateConnection(_options.ConnectionStringName);
            using var command = connection.CreateCommand();
            command.CommandText = _options.VrdbStatusStoredProcedure;
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(
                ...
                ex,
                "VRDB Status procedure execution failed for {StoredProcedure}, PnlDate {PnlDate}.",
                _options.VrdbStatusStoredProcedure,
                pnlDate);
            throw;
        }
    }
