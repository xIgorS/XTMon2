...
public sealed class NonXtgPortfolioRepository : INonXtgPortfolioRepository
{
    private readonly SqlConnectionFactory _connectionFactory;
    private readonly NonXtgPortfolioOptions _options;
    private readonly ILogger<NonXtgPortfolioRepository> _logger;

    public NonXtgPortfolioRepository(
        SqlConnectionFactory connectionFactory,
        IOptions<NonXtgPortfolioOptions> options,
        ILogger<NonXtgPortfolioRepository> logger)
    {
    ...
    public async Task<NonXtgPortfolioResult> GetNonXtgPortfolioAsync(DateOnly pnlDate, CancellationToken cancellationToken)
    {
        try
        {
            using var connection = _connectionFactory.CreateConnection(_options.ConnectionStringName);
            using var command = connection.CreateCommand();
            command.CommandText = _options.NonXtgPortfolioStoredProcedure;
            ...
            return new NonXtgPortfolioResult(
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(
                ...
                ex,
                "Non XTG Portfolio procedure execution failed for {StoredProcedure}, PnlDate {PnlDate}.",
                _options.NonXtgPortfolioStoredProcedure,
                pnlDate);
            throw;
        }
    }
