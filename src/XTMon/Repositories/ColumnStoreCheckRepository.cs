...
public sealed class ColumnStoreCheckRepository : IColumnStoreCheckRepository
{
    private readonly SqlConnectionFactory _connectionFactory;
    private readonly ColumnStoreCheckOptions _options;
    private readonly ILogger<ColumnStoreCheckRepository> _logger;

    public ColumnStoreCheckRepository(
        SqlConnectionFactory connectionFactory,
        IOptions<ColumnStoreCheckOptions> options,
        ILogger<ColumnStoreCheckRepository> logger)
    {
    ...
    public async Task<ColumnStoreCheckResult> GetColumnStoreCheckAsync(DateOnly pnlDate, CancellationToken cancellationToken)
    {
        try
        {
            using var connection = _connectionFactory.CreateConnection(_options.ConnectionStringName);
            using var command = connection.CreateCommand();
            command.CommandText = _options.ColumnStoreCheckStoredProcedure;
            ...
            return new ColumnStoreCheckResult(
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(
                ...
                ex,
                "Column Store Check procedure execution failed for {StoredProcedure}, PnlDate {PnlDate}.",
                _options.ColumnStoreCheckStoredProcedure,
                pnlDate);
            throw;
        }
    }
