...
public sealed class FutureCashRepository : IFutureCashRepository
{
    private readonly SqlConnectionFactory _connectionFactory;
    private readonly FutureCashOptions _options;
    private readonly ILogger<FutureCashRepository> _logger;

    public FutureCashRepository(
        SqlConnectionFactory connectionFactory,
        IOptions<FutureCashOptions> options,
        ILogger<FutureCashRepository> logger)
    {
    ...
    public async Task<FutureCashResult> GetFutureCashAsync(DateOnly pnlDate, CancellationToken cancellationToken)
    {
        try
        {
            using var connection = _connectionFactory.CreateConnection(_options.ConnectionStringName);
            using var command = connection.CreateCommand();
            command.CommandText = _options.FutureCashStoredProcedure;
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(
                ...
                ex,
                "Future Cash procedure execution failed for {StoredProcedure}, PnlDate {PnlDate}.",
                _options.FutureCashStoredProcedure,
                pnlDate);
            throw;
        }
    }
