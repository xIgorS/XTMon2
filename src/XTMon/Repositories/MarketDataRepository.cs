...
public sealed class MarketDataRepository : IMarketDataRepository
{
    ...
    public async Task<MarketDataResult> GetMarketDataAsync(DateOnly pnlDate, CancellationToken cancellationToken)
    {
        try
        {
            using var connection = _connectionFactory.CreateConnection(_options.ConnectionStringName);
            using var command = connection.CreateCommand();
            command.CommandText = _options.MarketDataStoredProcedure;
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(
                ...
                ex,
                "Market data procedure execution failed for {StoredProcedure} and PnlDate {PnlDate}.",
                _options.MarketDataStoredProcedure,
                pnlDate);
            throw;
        }
    }
