...
public sealed class SasTablesRepository : ISasTablesRepository
{
    private readonly SqlConnectionFactory _connectionFactory;
    private readonly SasTablesOptions _options;
    private readonly ILogger<SasTablesRepository> _logger;

    public SasTablesRepository(
        SqlConnectionFactory connectionFactory,
        IOptions<SasTablesOptions> options,
        ILogger<SasTablesRepository> logger)
    {
    ...
    public async Task<SasTablesResult> GetSasTablesAsync(DateOnly pnlDate, CancellationToken cancellationToken)
    {
        try
        {
            using var connection = _connectionFactory.CreateConnection(_options.ConnectionStringName);
            using var command = connection.CreateCommand();
            command.CommandText = _options.SasTablesStoredProcedure;
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(
                ...
                ex,
                "Sas tables procedure execution failed for {StoredProcedure}, PnlDate {PnlDate}.",
                _options.SasTablesStoredProcedure,
                pnlDate);
            throw;
        }
    }
